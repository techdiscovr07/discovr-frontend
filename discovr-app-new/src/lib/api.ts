...
export const creatorApi = {
    ...
    linkCampaign: (campaignId: string, creatorToken: string) => request<any>(`/creator/campaigns/link?campaign_id=${campaignId}&creator_token=${creatorToken}`, {
        method: 'POST'
    }),
    submitBid: (campaignId: string, amount: number) => request('/creator/campaigns/bid', {
        method: 'POST',
        body: JSON.stringify({ campaign_id: campaignId, amount })
    }),
    respondToBid: (campaignId: string, action: 'accept' | 'reject', proposedAmount?: number) => request<any>(`/creator/campaigns/bid/respond?campaign_id=${campaignId}`, {
        method: 'POST',
        body: JSON.stringify({ action, proposed_amount: proposedAmount })
    }),
